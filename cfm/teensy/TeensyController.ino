...
// # Analogue
#define P_A_LED_GCAMP 16
#define P_A_LED_OPTOGENETIC 17
...
#define STEPPER_MAXSPEED_Z 512.0
...
//        call_turnoff();
      ...
      if( subcmd == 's' || subcmd == 'S' ){ // Set Mode
        ...
        stepperX.setSpeed(STEPPER_MAXSPEED_X);
        stepperY.setSpeed(STEPPER_MAXSPEED_Y);
        stepperZ.setSpeed(STEPPER_MAXSPEED_Z);
...
//    Serial.send_now();
  ...
  if(x >  18000 && xspeed > 0){
    stepperX.setSpeed(0);
  }else if (x < -25000 && xspeed < 0){
    stepperX.setSpeed(0);
  }
  // # Y
  if(y >  18000 && yspeed > 0){
    stepperY.setSpeed(0);
  }else if (y < -25000 && yspeed < 0){
    stepperY.setSpeed(0);
  }
  // # Z
  if(z <=  -100 && zspeed < 0){
    stepperZ.setSpeed(0);
  }else if( z >= 20000 && zspeed > 0 ){
    stepperZ.setSpeed(0);
  }
