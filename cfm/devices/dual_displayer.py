...
class DualDisplayer:
    def __init__(
        ...
        self.dtype = np.uint8
        self.image_r = np.zeros(self.shape, self.dtype)
        self.image_g = np.zeros(self.shape, self.dtype)
        self.image = np.zeros((*self.shape, 3), self.dtype)
    ...
    def warp_r(self):
        return cv2.warpAffine(self.image_r, self.warp_matrix, *self.shape[::-1])
    ...
    def get_frame(self, combine=False):
        ...
        if combine:
            self.image[..., 2] = image_r // 2
            self.image[..., 1] = (image_r + self.image_g) // 2
            self.image[..., 0] = image_r // 2
            return image_r, self.image_g, self.image
        return image_r, self.image_g, None
